...
#### qk fixed models
class SelfAttention_qk_fixed(nn.Module):
    def __init__(self, in_dim, heads=8, dropout_rate=0.1):
        super(SelfAttention_qk_fixed, self).__init__()
        self.heads = heads
        self.head_dim = in_dim // heads
        self.scale = self.head_dim**0.5
...
class EncoderBlock_qk_fixed(nn.Module):
    def __init__(self,
                 in_dim,
                 mlp_dim,
                 num_heads,
                 dropout_rate=0.1,
                 attn_dropout_rate=0.1):
        super(EncoderBlock_qk_fixed, self).__init__()

        self.norm1 = nn.LayerNorm(in_dim)
        self.attn = SelfAttention_qk_fixed(in_dim,
                                  heads=num_heads,
                                  dropout_rate=attn_dropout_rate)
        if dropout_rate > 0:
            self.dropout = nn.Dropout(dropout_rate)
...
class Encoder_qk_fixed(nn.Module):
    def __init__(self,
                 num_patches,
                 emb_dim,
                 mlp_dim,
                 ...
                 dropout_rate=0.1,
                 attn_dropout_rate=0.0):
        super(Encoder_qk_fixed, self).__init__()
        ...
        self.encoder_layers = nn.ModuleList()
        for i in range(num_layers):
            layer = EncoderBlock_qk_fixed(in_dim, mlp_dim, num_heads, dropout_rate,
                                 attn_dropout_rate)
            self.encoder_layers.append(layer)
        self.norm = nn.LayerNorm(in_dim)
...
class VisionTransformer_qk_fixed(nn.Module):
    """ Vision Transformer.
        Most easily loaded with the .from_name or .from_pretrained methods.
        Args:
            params (namedtuple): A set of Params.
    ...
    """
    def __init__(self, params=None):
        super(VisionTransformer_qk_fixed, self).__init__()
        self._params = params
        ...
        # transformer
        self.transformer = Encoder_qk_fixed(
            num_patches=self.num_patches,
            emb_dim=self._params.emb_dim,
            mlp_dim=self._params.mlp_dim,
            num_layers=self._params.num_layers,
